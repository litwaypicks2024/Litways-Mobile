...
export default function FavoritesScreen() {
  ...
  const categories = useMemo(() => {
    ...
    for (const i of items) {
      ...
      counts.set(i.categorySlug, {
        name: i.categoryName ?? i.categorySlug,
        n: (c?.n ?? 0) + 1,
      });
    }
    ...
    try {
      ...
      if (r.moved.length && !skipped) {
        showToast({
          title:
            r.moved.length === 1
              ? "Moved 1 item to your cart"
              : `Moved ${r.moved.length} items to your cart`,
          tone: "success",
        ...
        alertDialog(
          ...
          lines.join(" ") || "Nothing here is available right now.",
          [{ text: "OK" }],
          "warning",
        );
      }
      ...
      <View style={{ flex: 1, backgroundColor: color.bg }}>
        ...
        >
          ...
          >
            ...
            <Text
              variant="title"
              style={{ textAlign: "center", marginTop: 16, marginBottom: 6 }}
            >
              Nothing saved yet
            </Text>
...
function Chip({
  ...
  return (
    ...
    >
      <Text
        variant="metaStrong"
        style={{ color: active ? color.onAccent : color.inkBody }}
      >
        {label}
      </Text>
    </TouchableOpacity>
